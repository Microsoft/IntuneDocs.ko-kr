...
* **ClientID**는 사용할 AAD ClientID(애플리케이션 ID라고도 함)입니다. Azure AD에 등록 된 경우 고유한 앱의 ClientID를 사용 하거나 ADAL을 통합 하지 않는 경우 [기본 등록](#default-enrollment-optional) 을 활용 해야 합니다.
...
MAM에 등록 된 사용자가 없지만 앱이 Android Enterprise 구성 (특정 사용자의 대상이 되지 않음)을 계속 검색 하려면 null 또는 빈 문자열을 전달 하면 됩니다.

### <a name="conflicts"></a>충돌
MAM 앱 구성에 설정 된 값은 Android Enterprise config에 설정 된 동일한 키를 가진 값을 재정의 합니다. 

관리자가 동일한 키에 대해 충돌 하는 값을 구성 하는 경우 (예: 동일한 사용자를 포함 하는 여러 그룹에 동일한 키를 사용 하 여 여러 앱 구성 집합을 대상으로 지정 하는 경우) Intune은이 충돌을 자동으로 해결 하 고 모든 값을 생성 합니다. 앱에서 사용할 수 있습니다. 

앱은 `MAMAppConfig` 개체에서 지정 된 키에 대 한 모든 값을 요청할 수 있습니다.
```java
...
Graph API를 사용 하 여 앱 구성을 구성할 수도 있습니다. 자세한 내용은 [MAM 대상 구성에 대 한 Graph API 문서](https://docs.microsoft.com/graph/api/resources/intune-mam-targetedmanagedappconfiguration)를 참조 하세요.
...
[테스트 가이드](app-sdk-android-testing-guide.md)를 참조 하세요.
