...
딥 링크는 공용 앱 스토어의 앱 목록에 대 한 링크입니다. 앱의 딥 링크가 향후에 변경되면 앱을 다시 등록해야 합니다.
...
* **Intune 앱 보호 정책**: 모든 Intune 앱 보호 정책에 대해 앱을 테스트하려면 각 정책 설정에 예상되는 동작이 무엇인지 알고 있어야 합니다. [iOS 앱 보호 정책](../apps/app-protection-policy-settings-ios.md) 및 [Android 앱 보호 정책](../apps/app-protection-policy-settings-android.md)에 대한 설명을 참조하세요. 앱이 Intune SDK를 통합 했지만 가능 apps 목록에 나열 되어 있지 않은 경우 ' 사용자 지정 앱 '을 선택할 때 텍스트 상자에 앱의 번들 ID (iOS) 또는 패키지 이름 (Android)을 지정할 수 있습니다. 
...
> [!NOTE]
> 앱이이 리소스에 액세스 하는 동안 오류가 발생 하 여 로그인 하지 못하도록 제한 하는 경우: https\://intunemam.microsoftonline.com, 앱의 클라이언트 ID를 사용 하 여 msintuneappsdk@microsoft.com에 대 한 메모를 전송 해야 합니다. 현재 수동 승인 프로세스입니다.
