...
description: MDM 관리 디바이스 사용, 회사 리소스에 대한 액세스 권한 얻기, iOS/iPadOS 디바이스의 오류 및 Microsoft Intune에서 OMA 응답 오류와 같은 경우 오류, 상태 코드, 설명 및 해결 방법의 목록을 확인합니다.
keywords: ''
...
## <a name="errors-returned-by-iosipados-devices"></a>iOS/iPadOS 디바이스가 반환한 오류
...
|__오류 발생__ <br>회사 포털 클라이언트를 로드할 수 없습니다. 다시 시도해 보고 문제가 계속되면 IT 관리자에게 문의하세요.|400 오류|HTTP 상태 코드가 400대이고 구체적인 오류 메시지가 없는 모든 오류에 대해 이 메시지가 표시됩니다. iOS/iPadOS 회사 포털 앱에서 발생하는 클라이언트쪽 오류입니다.|
...
|-2016345595|0x87D10205|Syncml(517): Atomic 명령에 대한 응답이 너무 커서 단일 메시지에 포함할 수 없습니다.|
|-2016345596|0x87D10204|Syncml(516): 명령이 Atomic 요소 내에 있는데 Atomic이 실패했습니다. 이 명령은 성공적으로 롤백되지 않습니다.|
|-2016345598|0x87D10202|Syncml(514): SyncML 명령이 처리되기 전에 작업이 이미 취소되었으므로 명령이 성공적으로 완료되지 않았습니다.|
|-2016345599|0x87D10201|Syncml(513): 받는 사람이 요청 SyncML 메시지에 사용된 SyncML 동기화 프로토콜의 지정된 버전을 지원하지 않거나 지원하기를 거부합니다.|
|-2016345600|0x87D10200|Syncml(512): 동기화 세션 동안 애플리케이션 오류가 발생했습니다.|
...
|-2016345603|0x87D101FD|Syncml(509): 나중에 사용하도록 예약되어 있습니다.|
|-2016345604|0x87D101FC|Syncml(508): 서버와 클라이언트의 현재 동기화 상태를 새로 고쳐야 하는 오류가 발생했습니다.|
|-2016345605|0x87D101FB|Syncml(507): 오류 때문에 Atomic 요소 유형 내의 모든 SyncML 명령이 실패했습니다.|
|-2016345606|0x87D101FA|Syncml(506): 요청을 처리하는 동안 애플리케이션 오류가 발생했습니다.|
|-2016345607|0x87D101F9|Syncml(505): 받는 사람이 요청 SyncML 메시지에 사용된 SyncML DTD의 지정된 버전을 지원하지 않거나 지원하기를 거부합니다.|
|-2016345608|=0x87D101F8|Syncml(504): 받는 사람이 게이트웨이 또는 프록시 역할을 하는 동안 URI(HTTP, FTP, LDAP 등)에 의해 지정된 업스트림 받는 사람 또는 요청의 완료를 시도할 때 액세스해야 하는 몇몇 다른 보조 받는 사람(DNS 등)의 응답을 적시에 받지 않았습니다.|
|-2016345609|0x87D101F7|Syncml(503): 받는 사람이 임시 오버로드 또는 받는 사람 유지 관리로 인해 현재 요청을 처리할 수 없습니다.|
|-2016345610|0x87D101F6|Syncml(502): 받는 사람이 게이트웨이 또는 프록시 역할을 하는 동안 요청의 이행을 시도할 때 액세스한 업스트림 받는 사람으로부터 잘못된 응답을 받았습니다.|
|-2016345611|0x87D101F5|Syncml(501): 받는 사람이 요청을 이행하는 데 필요한 명령을 지원하지 않습니다.|
|-2016345612|0x87D101F4|Syncml(500): 받는 사람이 요청의 이행을 방해하는 예기치 않은 조건을 발견했습니다.|
|-2016345684|0x87D101AC|Syncml(428): 이동 실패|
|-2016345685|0x87D101AB|Syncml(427): 부모가 자식 요소를 포함하고 있으므로 삭제할 수 없습니다.|
|-2016345686|0x87D101AA|Syncml(426): 부분 항목은 허용되지 않습니다.|
|-2016345687|0x87D101A9|Syncml(425): 보낸 사람이 받는 사람에 대한 적절한 액세스 제어 권한(ACL)을 가지고 있지 않기 때문에 요청한 명령이 실패했습니다.|
|-2016345688|0x87D101A8|Syncml(424): 청크 분할된 개체를 받았지만 받은 개체의 크기가 첫 번째 청크 내에 선언된 크기에 맞지 않습니다.|
|-2016345689|0x87D101A7|Syncml(423): "소프트 삭제" 항목이 이전에 서버에서 "하드 삭제"였으므로 요청한 명령이 실패했습니다.|
|-2016345690|0x87D101A6|Syncml(422): LocURI의 CGI 스크립트 형식이 잘못되었으므로 요청한 명령이 서버에서 실패했습니다.|
|-2016345691|0x87D101A5|Syncml(421): 지정된 검색 문법이 알려지지 않았기 때문에 요청한 명령이 서버에서 실패했습니다.|
|-2016345692|0x87D101A4|Syncml(420): 남아 있는 동기화 데이터를 위한 저장 공간이 받는 사람에게 더 이상 없습니다.|
|-2016345693|0x87D101A3|Syncml(419): 클라이언트 요청에서 충돌이 발생했으며 이 충돌이 우세한 서버 명령에 의해 해결되었습니다.|
|-2016345694|0x87D101A2|Syncml(418): 대상이 이미 있으므로 요청한 Put 또는 Add 명령이 실패했습니다.|
|-2016345695|0x87D101A1|Syncml(417): 요청이 이번에는 실패했으며 보낸 사람이 나중에 이 요청을 다시 시도해야 합니다.|
|-2016345696|0x87D101A0|Syncml(416): 요청의 지정된 바이트 크기가 너무 크기 때문에 요청이 실패했습니다.|
|-2016345697|0x87D1019F|Syncml(415): 지원되지 않는 미디어 유형 또는 형식입니다.|
|-2016345698|0x87D1019E|Syncml(414): 대상 URI가 너무 길어서 받는 사람이 처리할 수 없거나 처리를 원하지 않으므로 요청한 명령이 실패했습니다.|
|-2016345699|0x87D1019D|Syncml(413): 요청한 항목이 받는 사람이 처리할 수 있거나 처리를 원하는 것보다 크기 때문에 받는 사람이 요청한 명령의 수행을 거부합니다.|
|-2016345700|0x87D1019C|Syncml(412): 요청한 명령이 불완전하거나 잘못된 형식이므로 받는 사람에 대해 실패했습니다.|
|-2016345701|0x87D1019B|Syncml(411): 요청한 명령은 메타 요소 형식의 바이트 크기 또는 길이 정보를 함께 제공해야 합니다.|
|-2016345702|0x87D1019A|Syncml(410): 요청한 대상은 더 이상 받는 사람에 대한 것이 아니며 전달하는 URI를 알 수 없습니다.|
|-2016345703|0x87D10199|Syncml(409): 요청한 명령은 데이터의 클라이언트 버전과 서버 버전 사이의 업데이트 충돌 때문에 실패했습니다.|
|-2016345704|0x87D10198|Syncml(408): 예상되는 메시지가 필요한 기간 내에 수신되지 않았습니다.|
|-2016345705|0x87D10197|Syncml(407): 보낸 사람이 적절한 인증을 제공해야 하기 때문에 요청한 명령이 실패했습니다.|
|-2016345706|0x87D10196|Syncml(406): 요청의 옵션 기능이 지원되지 않으므로 요청한 명령이 실패했습니다.|
|-2016345707|0x87D10195|Syncml(405): 요청한 명령이 대상에서 허용되지 않습니다.|
|-2016345708|0x87D10194|Syncml(404): 요청한 대상을 찾을 수 없습니다.|
|-2016345709|0x87D10193|Syncml(403): 요청한 명령이 실패했지만 받는 사람이 요청한 명령을 인식했습니다.|
|-2016345710|0x87D10192|Syncml(402): 적절한 지불이 필요하기 때문에 요청한 명령이 실패했습니다.|
|-2016345711|0x87D10191|Syncml(401): 요청자가 적절한 인증을 제공해야 하기 때문에 요청한 명령이 실패했습니다.|
|-2016345712|0x87D10190|Syncml(400): 명령의 구문이 잘못된 형식이기 때문에 요청한 명령을 수행할 수 없습니다.|
|-2016345807|0x87D10131|Syncml(305): 요청한 대상은 지정된 프록시 URI를 통해 액세스해야 합니다.|
|-2016345808|0x87D10130|Syncml(304):요청한 SyncML 명령이 대상에서 실행되지 않았습니다.|
|-2016345809|0x87D1012F|Syncml(303): 요청한 대상을 다른 URI에서 찾을 수 있습니다.|
|-2016345810|0x87D1012E|Syncml(302): 요청한 대상이 일시적으로 다른 URI로 이동되었습니다.|
|-2016345811|0x87D1012D|Syncml(301): 요청한 대상에 새 URI가 있습니다.|
|-2016345812|0x87D1012C|Syncml(300): 요청한 대상은 대안으로 요청한 여러 대상 중 하나입니다.|
|-2016345896|0x87D100D8|Syncml(216): 명령이 Atomic 요소 내에 있는데 Atomic이 실패했습니다. 이 명령은 성공적으로 롤백되었습니다.|
|-2016345897|0x87D100D7|Syncml(215): 명령이 사용자 상호 작용의 결과로 그리고 사용자가 선택 항목에 동의하지 않았기 때문에 실행되지 않았습니다.|
|-2016345898|0x87D100D6|Syncml(214): 작업이 취소되었습니다. SyncML 명령은 성공적으로 완료되었지만 해당 세션 내에서 더 이상 명령이 처리되지 않습니다.|
|-2016345899|0x87D100D5|Syncml(213): 청크 분할된 항목이 허용되고 버퍼링됨|
|-2016345900|0x87D100D4|Syncml(212): 인증이 허용되었습니다. 동기화 세션의 나머지 부분에 대한 추가 인증은 필요하지 않습니다. 이 응답 코드는 자격 증명을 제공한 요청에 대한 응답에만 사용할 수 있습니다.|
