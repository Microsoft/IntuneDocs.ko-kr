...
2. **Windows Autopilot 디바이스 추가** 에서 추가할 디바이스를 나열하는 CSV 파일로 이동합니다. 파일에는 일련 번호, Windows 제품 ID, 하드웨어 해시 및 디바이스의 선택적 주문 ID가 나열되어야 합니다.
