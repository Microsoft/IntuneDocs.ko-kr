...
1. 관리 되는 Google Play 스토어에서 OEMConfig 앱을 가져옵니다. [Android 엔터프라이즈 디바이스에 관리되는 Google Play 앱 추가](apps-add-android-for-work.md)에서는 단계가 나열됩니다.
2. 일부 Oem은 OEMConfig 앱이 미리 설치 된 장치를 배송할 수 있습니다. 앱이 사전 설치 되지 않은 경우 Intune을 사용 하 여 [앱을 추가 하 고 장치에 배포](apps-deploy.md)합니다.
...
4. **연결 된 앱**을 선택 하 고 이전에 추가한 기존 oemconfig 앱을 선택 > **확인**합니다. 정책을 할당 하는 장치에 대 한 올바른 OEMConfig 앱을 선택 해야 합니다.
    ...
    나열 된 앱이 표시 되지 않는 경우 관리 Google Play를 설정 하 고 관리 되는 Google Play 저장소에서 앱을 가져옵니다. [Android 엔터프라이즈 디바이스에 관리되는 Google Play 앱 추가](apps-add-android-for-work.md)에서는 단계가 나열됩니다.
