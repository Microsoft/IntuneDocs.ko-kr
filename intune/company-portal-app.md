...
|필드 이름|추가 정보|
|---|---|
|**표준 색을 선택하거나 6자리 16진수 코드 입력**| 특정 색을 선택하려면 시각적 개체에 **표준**을 선택합니다. 16진수 코드 값을 기준으로 특정 색을 선택하려면 **사용자 지정**을 선택합니다.|
...
회사 브랜드를 반영하는 브랜드 이미지를 표시합니다. 브랜드 이미지에 대한 지원이 회사 포털 앱에 추가되지만 모든 플랫폼에서 표시되지 않을 수 있습니다.

|필드 이름|추가 정보|
|---|---|
|**브랜드 이미지 업로드**| 이 옵션을 사용하면 회사 포털 앱의 사용자 프로필 페이지에 배경 이미지를 표시할 수 있습니다.<p><ul><li>권장 이미지 너비: 640px~1125px</li><li>최대 이미지 크기: 1.3MB</li><li>파일 형식: PNG, JPG 또는 JPEG</li></ul>|

오른쪽 브랜드 이미지는 회사 브랜드의 강력한 인상을 심어주어 회사 포털에 대한 사용자의 신뢰를 높여줄 수 있습니다. 회사 포털의 이미지를 가져오고, 선택하고, 최적화할 때 고려하면 좋은 몇 가지 팁이 있습니다. 

- 마케팅 또는 아트 부서에 연락합니다. 브랜딩된 브랜드 이미지의 승인된 집합을 이미 보유하고 있을 수 있습니다. 또한 필요에 따라 이미지를 최적화하는 데 도움을 받을 수도 있습니다. 

- 가로 및 세로 구도를 모두 고려하세요. 이미지의 초점 주변에는 충분한 배경이 있어야 합니다. 장치 크기 및 방향에 따라 이미지가 다르게 잘릴 수 있습니다. 
...
변경 내용이 저장되면 Intune 웹 포털의 블레이드 위쪽에서 **설정 미리 보기**를 선택하여 구성의 모양을 확인할 수 있습니다. iOS 장치에 표시되는 브랜드 이미지만 미리 볼 수 있습니다(Intune 웹 포털은 불가). 
