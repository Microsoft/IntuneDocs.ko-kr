...
Windows 10을 실행 하는 장치용 Microsoft Intune에서 지원 되는 중이거나 이상 MDM 보안 기준 설정을 확인 합니다. 이 기준선의 설정에 대 한 기본값을 적용 가능한 장치에 대 한 권장된 구성을 나타내도록 및 다른 보안 기준에서 초기 기본값 일치 하지 않을 수 있습니다.  

최신 기준 버전은 **Spring 2019 업데이트 (19 시간 1)에 대 한 MDM 보안 기준**  
...
- **상승 된 권한으로 블록 MSI 앱 설치**  
  ...
  - *이 정책 설정을 사용 하도록 설정 하면*, 모든 프로그램에 권한을 확장 됩니다. 이 사용 권한은 일반적으로 컴퓨터 (자동으로 설치) f에 할당 된 사용자 (바탕 화면에서 제공)에 할당 되거나 제어판의 프로그램 추가 / 제거에서 사용할 수 있는 프로그램에 대 한 예약 합니다. 이 프로필 설정은 사용자 권한 보기 또는 변경, 매우 제한 된 컴퓨터의 디렉터리를 포함 하 여 없을 디렉터리에 대 한 액세스를 필요로 하는 프로그램을 설치할 수 있도록 합니다.
  - *사용 하지 않거나이 정책 설정을 구성 하지 않은 경우*, 시스템 관리자가 배포 하지 않거나 제공 하는 프로그램을 설치할 때 현재 사용자의 권한을 적용 합니다. 참고: 이 정책 설정은 컴퓨터 구성 및 사용자 구성 폴더 모두에 표시됩니다. 이 정책 설정을 적용 하기를 위해 두 폴더 모두에서 활성화 해야 합니다. 주의: 숙련 된 사용자 수 이용 권한을이 정책 설정을 해당 권한을 변경 하 여 제한 된 파일 및 폴더에 대 한 영구 액세스 권한을 부여 합니다. 이 정책 설정은 사용자 구성 버전 보안 되도록 보장 되지 않는 참고 합니다.  
...
- **커널 DMA 보호와 호환되지 않는 외부 디바이스 열거**  
  이 정책은 외부 DMA 가능 장치에 대 한 추가 보안을 제공 합니다. 이 정책을 통해 DMA 다시 매핑/디바이스 메모리 격리 및 샌드박스와 호환되지 않는 외부 DMA 지원 디바이스의 열거를 더 세부적으로 제어할 수 있습니다. 이 정책은 커널 DMA 보호가 지원되고 시스템 펌웨어를 통해 사용하도록 설정되는 경우에만 적용됩니다. 커널 DMA 보호는 플랫폼 기능을 최종 사용자 또는 정책을 통해 제어할 수 없습니다. 이 기능은 제조 시 시스템에서 지원되어야 합니다. 시스템에서 커널 DMA 보호를 지원 하는 경우를 확인 하려면 MSINFO32.exe의 요약 페이지의 커널 DMA 보호 필드를 확인 하세요.  
...
- **Internet Explorer 암호화 지원**  
  이 정책 설정은 사용 하면 브라우저에서 전송 계층 보안 (TLS) 1.0, TLS 1.1, TLS 1.2, SSL Secure Sockets Layer () 2.0 또는 SSL 3.0 지원을 해제할 수 있습니다. TLS 및 SSL는 브라우저 및 대상 서버 간의 통신을 보호 하는 데 도움이 되는 프로토콜입니다. 브라우저를 대상 서버와 보호 된 통신을 설정 하려고 하는 경우 브라우저와 서버는 프로토콜 및 버전을 사용 하 여 협상 합니다. 지원 되는 프로토콜 및 버전을 다른 사용자의 목록과 일치 하도록 시도 하는 브라우저와 서버 및 우선 순위가 가장 낮은 일치 항목을 선택 합니다. 이 정책 설정을 사용 하도록 설정 하면 브라우저 협상 또는 드롭 다운 목록에서 선택 하는 암호화 방법을 사용 하 여 암호화 터널을 협상 하지 않습니다. 사용 하지 않도록 설정 하거나이 정책 설정을 구성 하지 않은 경우를 선택할 수 있는 암호화 메서드는 브라우저를 지원 합니다.  
  [자세한 정보](https://go.microsoft.com/fwlink/?linkid=2067057)

  **기본**: 2 개 항목: TLS v1.1 및 TLS v1.2  
  *이 설정에 대해 선택할 수 있는 옵션을 표시 하려면 아래쪽 화살표를 선택 합니다.*
...
- **NTLM SSP 기반 서버에 대한 최소 세션 보안**  
  이 보안 설정을 사용하면 서버에서는 128비트 암호화 및/또는 NTLMv2 세션 보안의 협상을 요구할 수 있습니다. 이러한 값은 LAN 관리자 인증 수준 보안 설정 값에 따라 달라집니다. 옵션은 다음과 같습니다. NTLMv2 세션 보안 필요: 메시지 무결성이 협상되지 않으면 연결이 실패합니다. 128비트 암호화가 필요합니다. 강력한 암호화(128비트)가 협상되지 않으면 연결이 실패합니다.  
...
- **원격 지원 요청**  
  이 정책 설정을 설정 하거나이 컴퓨터에서 원격 지원 요청 (에 대 한 질문) 해제할 수 있습니다. 
  - *이 정책 설정을 사용 하도록 설정 하면*,이 컴퓨터에 사용자 도움말에 대 한 사용자에 게 전자 메일 또는 파일 전송을 사용할 수 있습니다. 또한 사용자가이 컴퓨터에 연결할 수 있도록 인스턴트 메시징 프로그램을 사용할 수 있으며 추가 원격 지원 설정을 구성할 수 있습니다. 
  - *이 정책 설정을 사용 하지 않으면*,이 컴퓨터에 사용자 도움말에 대 한 사용자에 게 전자 메일 또는 파일 전송을 사용할 수 없습니다. 또한 사용자는이 컴퓨터에 연결할 수 있도록 인스턴트 메시징 프로그램을 사용할 수 없습니다. 
  - *이 정책 설정을 구성 하지 않으면*, 사용자 설정 하거나 해제할 수 있습니다 (도움 요청) 요청 된 원격 지원 자체 제어판의 시스템 속성에서입니다. 사용자가 원격 지원 설정을 구성할 수도 있습니다. 

  이 정책 설정을 사용 하도록 설정 하면 두 가지 방법이 있습니다 도우미 원격 지원을 제공할 수 있도록 합니다. "만 보기로 도우미 컴퓨터 허용" 또는 "를 원격으로 컴퓨터를 제어할 수 있도록 합니다." "최대 티켓 시간" 정책 설정 제한 시간에는 전자 메일 또는 파일 전송을 사용 하 여 만든 원격 지원 요청을에 열려 있을 수 있습니다. "전자 메일 초대를 보내는 방법 선택" 원격 지원 요청을 전송 하는 데는 전자 메일 표준 지정 합니다. 전자 메일 프로그램에 따라 (초대를 받는 사람 인터넷 링크를 통해 연결) 표준 Mailto 또는 SMAPI (Simple MAPI) 표준 (초대는 전자 메일 메시지에 연결 됨)를 사용할 수 있습니다. SMAPI는 지원 되는 유일한 방법 이므로이 정책 설정은 Windows Vista에서 사용할 수 없는 경우 이 정책 설정을 사용 하도록 설정 하면 원격 지원 통신을 허용 하도록 적절 한 방화벽 예외에도 사용 해야 합니다.  
  [자세한 정보](https://go.microsoft.com/fwlink/?linkid=2067198)

  **기본**: 원격 지원을 사용 하지 않도록 설정

  로 설정 하면 *원격 지원을 사용 하도록 설정*, 다음과 같은 추가 설정을 구성 합니다.  
  - **원격 지원 요청 권한**  
    **기본값**: 보기  
...
- **동작 모니터링**  
  Windows Defender 동작 모니터링 기능을 허용할지 여부를 지정 합니다. Windows 10에 포함된 이러한 센서는 운영 체제에서 동작 신호를 수집하고 처리하며, 이 센서 데이터를 Microsoft Defender ATP의 격리된 프라이빗 클라우드 인스턴스로 보냅니다.  
...
자세한 내용은 [2.2.2 FW_PROFILE_TYPOE]( https://docs.microsoft.com/openspecs/windows_protocols/ms-fasp/7704e238-174d-4a5e-b809-5f3787dd8acc) Windows 프로토콜 문서에 있습니다.  
...
- **사용 가능한 경우 향상된 스푸핑 방지 필요**  
  그러한 경우 향상 된 스푸핑 방지 사용 가능한 경우 장치를 사용 합니다. 아니요 이면 스푸핑 방지 차단 됩니다. 가 구성 되지 않은 클라이언트에서 수행한 구성이 적용 됩니다.  
...
- **PIN에 특수 문자 필요**  
  필요한 경우 사용자 PIN에 특수 문자를 하나 이상 포함 해야 합니다.

  **기본값**: 허용됨

- **최소 PIN 길이**  
  최소 PIN 길이 4에서 127 사이 여야 합니다.
...
## <a name="whats-changed-in-the-new-template"></a>새 템플릿 변경 사항
*Spring 2019 업데이트 (19 시간 1)에 대 한 MDM 보안 기준* 템플릿에에서 다음과 같이 변경 합니다 *미리 보기* 템플릿.

### <a name="changes-to-the-baseline-settings"></a>초기 설정의 변경
설정은 다음 중 하나입니다.
- *새* 기준의이 최신 버전에서입니다.
- *제거* 이 최신 기준선 버전 이지만 있던 이전 버전에서입니다.
...
- 비트 보관 이동식 드라이브 정책 > **암호화 메서드**
- **Bitlocker 고정 드라이브 정책과** *(모든 설정)*
- **비트 보관 시스템 드라이브 정책** *(모든 설정)*

*[새로운 기능]* [**연결**](#connectivity):
- **UNC 경로에 대 한 보안 액세스를 구성 합니다.**
...
- **Internet Explorer 인터넷 영역 파일 다운로드에 대 한 자동 프롬프트** >의 기본값은 이제 **비활성**합니다. 미리 보기에서이 사용으로 설정 되었습니다.
