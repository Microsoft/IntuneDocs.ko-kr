...
- **기본**: 반기 채널(대상 지정)  
- **Windows 참조 설명서**: [업데이트/BranchReadinessLevel](https://docs.microsoft.com/windows/client-management/mdm/policy-csp-update#update-branchreadinesslevel)  
디바이스가 Windows 업데이트를 받을 채널(분기)을 설정합니다. 업데이트가 전달되기 전에 채널에 따라 사용되는 지연 기간이 서로 다를 수 있습니다.  

예를 들어 반기 채널에는 6개월 지연이 포함됩니다. 즉, 이 설정 본문에서 추가 지연 없이 이 채널을 사용하는 경우, 디바이스는 릴리스된 지 6개월 후에 업데이트를 설치합니다.  
...
- **기본**: 허용
- **Windows 참조 설명서**: [업데이트/AllowMUUpdateService](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-allowmuupdateservice)
...
- **기본**: 허용
- **Windows 참조 설명서**: [업데이트/ExcludeWUDriversInQualityUpdate](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-excludewudriversinqualityupdate)
...
- **기본값**: 0  
- **Windows 참조 설명서**: [업데이트/DeferQualityUpdatesPeriodInDays](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-deferqualityupdatesperiodindays)  
...
- **기본값**: 0  
- **Windows 참조 설명서**: [업데이트/PauseFeatureUpdatesPeriodInDays](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-deferfeatureupdatesperiodindays)  

기능 업데이트가 지연되는 기간(일)을 지정합니다. 이 기간은 선택한 서비스 채널의 일부인 지연 기간에 추가됩니다. 디바이스에서 정책을 수신하면 지연 기간이 시작됩니다.  
지원되는 지연 기간:  

- *Windows 버전 1709 이상을*: 0-365 일  
- *Windows 버전 1703*: 0 ~ 180 일  
...
- **기본값**: 10  
- **Windows 참조 설명서**: [업데이트/ConfigureFeatureUpdateUninstallPeriod](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-configurefeatureupdateuninstallperiod)  
...
- **Windows 참조 설명서**: [Update/AllowAutoUpdate](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-allowautoupdate)  
...
- **유지 관리 시 자동 설치** – 업데이트가 자동으로 다운로드된 후 디바이스가 사용되지 않거나 절전 모드로 실행되는 자동 유지 관리 중에 설치됩니다. 다시 시작이 필요한 경우, 최대 7일 동안 다시 시작할지 묻는 메시지가 사용자에게 표시된 후 다시 시작이 적용됩니다.  
  ...
  - **활성 시간 시작**: 업데이트 설치로 인한 다시 시작을 차단하기 위한 시작 시간을 지정합니다.  
    **Windows 참조 설명서**: [업데이트/ActiveHoursStart](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-activehoursstart)  
    **기본**: 오전 8시  
  
  - **활성 시간 종료**: 업데이트 설치로 인한 다시 부팅을 차단하기 위한 종료 시간을 지정합니다.  
    **Windows 참조 설명서**: [업데이트/ActiveHoursEnd](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-activehoursend)  
    **기본**: 오후 5 시  
...
- **예약된 시간에 자동 설치 및 다시 시작** – 설치 날짜 및 시간을 지정합니다. 이 옵션을 지정하지 않으면, 매일 오전 3시에 설치가 실행되고, 이후 다시 시작될 때까지 15분씩 카운트다운됩니다. 로그온한 사용자는 카운트다운 및 다시 시작을 연기할 수 있습니다.  
  ...
  **Windows 참조 설명서**: [Update/AllowAutoUpdate](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-allowautoupdate)  
  ...
  - **예약 된 설치 날짜**: 하려는 업데이트를 설치 하는 요일 지정 합니다.  
    **기본**: 임의의 날짜  

  - **예약 된 설치 시간**: 업데이트를 설치 하려는 경우 하루 중 시간을 지정 합니다.  
    **기본**: 오전 3 시  
...
- **기본**: 허용  
- **Windows 참조 설명서**: [업데이트/SetEDURestart](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-setedurestart)  
...
- **기본**: 구성 되지 않음  
- **Windows 참조 설명서**: [업데이트/AutoRestartRequiredNotificationDismissal](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-autorestartrequirednotificationdismissal)
...
- **기본**: *이 옵션은 기본적으로 구성되지 않으며 사용자에게 미리 알림이 제공되지 않습니다.*  
- **Windows 참조 설명서**: [업데이트/ScheduleRestartWarning](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-schedulerestartwarning)  
...
- **기본**: *이 옵션은 기본적으로 구성되지 않으며 사용자에게 미리 알림이 제공되지 않습니다.*  
- **Windows 참조 설명서**: [업데이트/ScheduleImminentRestartWarning](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-scheduleimminentrestartwarning) 
...
- **기본**: 구성 되지 않음  
- **Windows 참조 설명서**: *적용할 수 없음*  
- **Windows 버전**: 지원에 대 한 Windows 10 버전 1803 이상  
...
이 옵션에 대한 자세한 내용은 업데이트 배포에 대한 Windows 10 설명서에서 [개입형 다시 시작](https://docs.microsoft.com/en-us/windows/deployment/update/waas-restart#engaged-restart)을 참조하세요.  

다음 설정은 개입형 다시 시작 작업이 발생하는 시기를 제어하는 데 사용됩니다.  

- **자동 다시 시작 후 사용자를 개입형 다시 시작으로 전환(일)**  
  - **기본**: 기본적으로 이 옵션은 구성되어 있지 않지만 **2**~**30** 사이의 값을 지원합니다.  
  - **Windows 참조 설명서**: [업데이트/EngagedRestartTransitionSchedule](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-engagedrestarttransitionschedule)  
...
- **개입형 다시 시작 미리 알림 다시 알림(일)**  
  - **기본**: 기본적으로 이 옵션은 구성되어 있지 않지만 **1**~**3** 사이의 값을 지원합니다.  
  - **Windows 참조 설명서**: [업데이트/EngagedRestartSnoozeSchedule](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-engagedrestartsnoozeschedule)  
...
- **보류 중인 다시 시작에 대한 최종 기한 설정(일)**  
  - **기본**: 기본적으로 이 옵션은 구성되어 있지 않지만 **2**~**30** 사이의 값을 지원합니다.  
  - **Windows 참조 설명서**: [업데이트/EngagedRestartDeadline](https://docs.microsoft.com/en-us/windows/client-management/mdm/policy-csp-update#update-engagedrestartdeadline)  
