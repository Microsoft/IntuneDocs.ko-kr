...
- **Windows Server**: Microsoft Intune Certificate Connector(NDESConnectorSetup.exe) 호스트
...
14. 서버에서 Intune 등록 장치 및 사용자를 대신하여 인증서를 관리하려면 아래 단계를 따릅니다.
    ...
    2. [보안] 탭에서 Microsoft Intune Certificate Connector를 실행할 서버의 컴퓨터 계정을 추가합니다. **인증서 발급 및 관리** 및 **인증서 요청** 허용 권한을 컴퓨터 계정에 부여합니다.
...
5. 다운로드가 완료되면 서버에 로그인합니다. 그런 다음:
    ...
    2. 설치 프로그램(NDESConnectorSetup.exe)을 실행하고 기본 위치를 적용합니다. 커넥터를 `\Program Files\Microsoft Intune\NDESConnectorUI\NDESConnectorUI.exe`에 설치합니다. 설치 관리자 옵션에서 **PFX 배포**를 선택합니다. 계속하여 설치를 완료합니다.

6. NDES 커넥터가 **등록** 탭을 엽니다. Intune에 대한 연결을 사용하려면 **로그인**하고 전역 관리자 권한이 있는 계정을 입력합니다.
7. **고급** 탭에서 **이 컴퓨터의 시스템 계정 사용(기본값)** 을 선택한 상태로 둡니다.
