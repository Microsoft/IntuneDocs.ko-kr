...
- **잉크 작업 영역** - 사용자가 잉크 작업 영역에 액세스하는 것을 차단 합니다. **구성되지 않음**으로 설정하면 잉크 작업 영역이 활성화되고(기능이 켜짐), 사용자는 잠금 화면 위에서 이를 사용할 수 있습니다.
- **자동 재배포** - 관리 권한을 가진 사용자가 장치 잠금 화면에서 **CTRL + Win + R**을 사용하여 모든 사용자 데이터 및 설정을 삭제할 수 있습니다. 장치가 자동으로 재구성되고 관리에 다시 등록됩니다.
- **사용자가 디바이스를 설치하는 동안 네트워크에 연결하도록 요구(Windows Insider에만 해당)** - Windows 10을 설치 중에 네트워크 페이지를 지나 계속 진행하기 전에 디바이스를 네트워크에 연결하도록 **필요**를 선택합니다. 이 기능이 미리 보기 상태인 동안 이 설정을 사용하려면 Windows Insider build 1809 이상이 필요합니다.
...
- **사용자 작업 게시**: **블록**을 통해 작업 전환기에서 최근에 사용한 리소스의 공유 경험 및 검색을 방지할 수 있습니다.
- **로컬 작업 전용**: **블록**을 통해 로컬 작업에만 기반한 작업 전환기에서 최근에 사용한 리소스의 공유 경험 및 검색을 방지할 수 있습니다.
...
- **신뢰할 수 있는 디바이스** - 이 앱에서 신뢰할 수 있는 디바이스(이미 연결되었거나 이 PC, 태블릿 또는 휴대폰과 함께 제공되는 하드웨어)를 사용할 수 있는지 여부를 선택합니다. 예를 들어 TV, 프로젝터 등이 있습니다.
...
-   **개인 저장소만 사용** - 최종 사용자만 개인 저장소에서 앱을 다운로드하도록 허용합니다.
...
-   **Microsoft Edge 브라우저(모바일 전용)** - 디바이스에서 Microsoft Edge 웹 브라우저 사용을 허용합니다.
-   **주소 표시줄 드롭다운(데스크톱 전용)** – 입력 시 Microsoft Edge에서 드롭다운 목록에 제안 목록을 표시하지 않습니다. 이 옵션을 사용하면 Microsoft Edge와 Microsoft 서비스 간의 네트워크 대역폭 사용을 최소화할 수 있습니다.
-   **Microsoft 브라우저 간 즐겨찾기 동기화(데스크톱 전용)** – Windows에서 Internet Explorer와 Microsoft Edge 간에 즐겨찾기를 동기화할 수 있도록 합니다.
-   **추적 방지 헤더 보내기** - 사용자가 방문하는 웹 사이트에 추적 방지 헤더를 보내도록 Microsoft Edge 브라우저를 구성합니다.
-   **쿠키** - 브라우저에서 장치에 인터넷 쿠키를 저장할 수 있도록 합니다.
-   **JavaScript** - Javascript와 같은 스크립트를 Microsoft Edge 브라우저에서 실행할 수 있습니다.
...
-   **개발자 도구** - 최종 사용자가 Microsoft Edge 개발자 도구를 열지 못하도록 합니다.
-   **확장** - 최종 사용자가 디바이스에 Microsoft Edge 확장을 설치할 수 있습니다.
-   **InPrivate 브라우징** - 최종 사용자가 InPrivate 브라우징 세션을 열 수 없습니다.
-   **처음 실행 페이지 표시** - Microsoft Edge를 처음 실행할 때 소개 페이지가 표시되지 않도록 합니다.
    -   **첫 실행 URL** – 사용자가 Microsoft Edge를 처음 실행할 때 표시되는 페이지의 URL을 지정합니다(Windows 10 Mobile에만 해당).
-   **홈페이지** - Microsoft Edge 브라우저의 홈페이지로 사용할 사이트 목록을 추가합니다(데스크톱 전용).
-   **시작 페이지 변경 내용** – 사용자가 Microsoft Edge를 열 때 표시되는 시작 페이지를 변경하도록 허용합니다. 홈페이지 설정을 사용하여 Microsoft Edge를 시작할 때 열리는 페이지 또는 페이지 목록을 만들 수 있습니다.
-   **About 플래그에 대한 액세스 차단** - 최종 사용자가 개발자 및 실험 설정이 포함된 Microsoft Edge의 about:flags 페이지에 액세스할 수 없도록 합니다.
...
-   **종료 시 검색 데이터 지우기** – 사용자가 Microsoft Edge를 종료할 때 기록 및 검색 데이터를 지웁니다.
-   **라이브 타일 데이터 수집** – 사용자가 Microsoft Edge의 시작 메뉴에 사이트를 고정할 때 Windows가 라이브 타일에서 정보를 수집할 수 없도록 차단합니다.
-  **즐겨찾기 목록** - 즐겨찾기 파일의 경로를 정의 합니다. 정의합니다(예: http://contoso.com/favorites.html).
-  **즐겨찾기 변경 제한** - **블록**을 통해 사용자가 즐겨찾기 목록 추가, 가져오기, 정렬 또는 편집하지 못하게 합니다. 
...
- **작업 표시줄에서 앱 고정 해제** - **블록**을 선택하여 사용자가 시작 메뉴에서 앱 고정을 해제하지 못하게 합니다.
- **빠른 사용자 전환** - **블록**을 선택하여 로그오프하지 않고 동시에 로그온한 사용자 간의 전환을 방지합니다.
- **가장 많이 사용된 앱** - **블록**을 선택하여 가장 많이 사용된 앱이 시작 메뉴에 표시되지 않도록 숨깁니다. 또한 설정 앱에서 해당 토글을 비활성화합니다.
- **최근에 추가된 앱** - **블록**을 선택하여 최근에 추가된 앱이 시작 메뉴에 표시되지 않도록 숨깁니다. 또한 설정 앱에서 해당 토글을 비활성화합니다.
- **시작 화면 모드** - 시작 화면이 표시되는 방법을 선택합니다. **전체 화면** 또는 **비전체 화면**으로 표시하도록 선택합니다.
- **점프 목록에서 최근에 연 항목** - **블록**을 선택하여 시작 메뉴 및 작업 표시줄에 최근 점프 목록이 표시되지 않도록 숨깁니다. 또한 설정 앱에서 해당 토글을 비활성화합니다.
- **앱 목록** - 설정 앱의 표시 방법을 선택합니다. 옵션은 다음과 같습니다. 
  - 축소
  - 설정 앱 축소 및 사용 안 함 
  - 설정 앱 제거 및 사용 안 함
- **전원 단추** - **블록**을 선택하여 시작 메뉴의 전원 단추가 표시되지 않도록 숨깁니다.
- **사용자 타일** - **블록**을 선택하여 시작 메뉴의 사용자 타일이 표시되지 않도록 숨깁니다.
  - **잠금** - **블록**을 선택하여 시작 메뉴의 사용자 타일에 `Lock` 옵션이 표시되지 않도록 숨깁니다.
  - **로그아웃** - **블록**을 선택하여 시작 메뉴의 사용자 타일에 `Sign out` 옵션이 표시되지 않도록 숨깁니다.
- **종료** - **블록**을 선택하여 시작 메뉴의 전원 단추에 `Update and shut down` 및 `Shut down` 옵션이 표시되지 않도록 숨깁니다.
- **절전** - **블록**을 선택하여 시작 메뉴의 전원 단추에 `Sleep` 옵션이 표시되지 않도록 숨깁니다.
- **최대 절전 모드** - **블록**을 선택하여 시작 메뉴의 전원 단추에 `Hibernate` 옵션이 표시되지 않도록 숨깁니다.
- **계정 전환** - **블록**을 선택하여 시작 메뉴의 사용자 타일에 `Switch account`가 표시되지 않도록 숨깁니다.
- **다시 시작 옵션** - **블록**을 선택하여 시작 메뉴의 전원 단추에 `Update and restart` 및 `Restart` 옵션이 표시되지 않도록 숨깁니다.
...
- **Azure 네이티브 클라이언트 앱 GUID** - OAuthAuthority에서 OAuth 토큰을 가져올 권한이 있는 클라이언트 애플리케이션의 GUID를 입력합니다.
