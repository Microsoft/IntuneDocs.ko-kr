...
|**Managed Browser에서 표시할 수 있는 웹 콘텐츠 제한** | 앱의 웹 링크를 Managed Browser 앱에서 열 수 있도록 적용하려면 **예**를 선택합니다. <br><br> Intune에 등록되지 않은 장치의 경우 Managed Browser 앱에서만 정책 관리 앱의 웹 링크를 열 수 있습니다. <br><br> Intune을 사용하여 장치를 관리하는 경우 [Microsoft Intune에서 Managed Browser 정책을 사용하여 인터넷 액세스 관리](app-configuration-managed-browser.md)를 참조하세요. <br><br> 모바일 장치(iOS 및 Android)용 Microsoft Edge 브라우저는 Intune 앱 보호 정책을 지원합니다. Edge 브라우저 응용 프로그램에서 회사 Azure AD 계정으로 로그인하는 사용자는 Intune에서 보호됩니다. Edge 브라우저는 MAM SDK를 통합하고, 다음과 같은 방지를 제외하고는 모든 데이터 보호 정책을 지원합니다.<ul><li>**다른 이름으로 저장**: Edge 브라우저에서 사용자가 클라우드 저장소 공급자(예: OneDrive)에 직접 앱 내 연결을 추가할 수 없고, 파일을 로컬 파일 시스템에 다운로드하지 못하도록 합니다.</li><li>**연락처 동기화**: Edge 브라우저는 기본 연락처 목록에 저장하지 않습니다.</li></ul>Edge 브라우저는 Intune 정책에 대한 자동 등록을 이용합니다. 즉, 장치의 다른 Microsoft 응용 프로그램이 Intune 정책으로 관리되면 Edge 브라우저에서 자동으로 Edge 앱을 대상으로 하는 정책이 있는지 확인합니다. Intune 정책에 대한 자동 등록은 MAM SDK에서 처리하므로 앱이 참여할 필요가 없습니다. | 아니요 |
