...
description: Intune 및 Microsoft Defender Security Center에서 Microsoft Defender ATP를 켜는 경우와 같은 엔드투엔드 시나리오에서 Microsoft Defender Advanced Threat Protection(Microsoft Defender ATP)을 사용하도록 설정하고, Microsoft Defender ATP 구성 프로필을 사용하여 디바이스를 온보딩하고, Intune 디바이스 준수 정책을 만들고, Azure AD 조건부 액세스 정책을 만들고, 디바이스 준수를 모니터링하는 방법을 확인합니다.
keywords: ''
author: brenduns
ms.author: brenduns
manager: dougeby
...
Microsoft Defender Advanced Threat Protection(Microsoft Defender ATP) 및 Microsoft Intune은 함께 작동하여 보안 위반을 방지하고 조직 내에서 위반 영향을 제한합니다.
...
Microsoft Defender ATP는 이 시나리오와 같은 보안 이벤트를 해결할 수 있습니다. Microsoft Defender Security Center는 디바이스를 “높은 위험”으로 보고하고 의심스러운 활동의 자세한 보고서를 포함합니다. 이 예제에서 Microsoft Defender ATP는 디바이스에서 비정상적인 코드가 실행되었으며, 프로세스 권한 에스컬레이션이 발생하고 악성 코드가 삽입되고 의심스러운 원격 셸이 실행되었음을 감지합니다. 그런 다음, Microsoft Defender ATP는 위협을 완화하는 옵션을 제공합니다.
...
- Microsoft Defender Security Center에서 Intune을 사용하도록 설정하고 Intune에서 Microsoft Defender ATP를 사용하도록 설정합니다. 이러한 작업은 Intune과 Microsoft Defender ATP 간에 서비스-서비스 연결을 만듭니다. 이 연결을 통해 Microsoft Defender ATP에서 Intune 디바이스에 대한 컴퓨터 위험을 작성할 수 있습니다.
...
일반적으로 이 작업은 한 번 수행합니다. 따라서 Intune 리소스에서 이미 Microsoft Defender ATP를 사용하도록 설정한 경우에는 다시 이 작업을 수행하지 않아도 됩니다.

## <a name="onboard-devices-using-a-configuration-profile"></a>구성 프로필을 사용하여 디바이스 등록

최종 사용자가 Intune에 등록하면 구성 프로필을 사용하여 다른 설정을 디바이스에 푸시할 수 있습니다. 이는 Microsoft Defender ATP에도 적용됩니다.

Microsoft Defender ATP에는 [Microsoft Defender ATP 서비스](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/microsoft-defender-advanced-threat-protection)와 통신하여 파일을 검색하고, 위협을 탐지하며, Microsoft Defender ATP에 위험을 보고하는 온보딩 구성 패키지가 포함되어 있습니다.

온보딩하면 Intune에서 Microsoft Defender ATP를 통해 자동 생성된 구성 패키지를 가져옵니다. Intune은 디바이스로 구성 프로필을 보낼 때 디바이스에 구성 패키지를 푸시하여 Microsoft Defender ATP가 해당 디바이스에서 위협이 있는지 모니터링할 수 있도록 합니다.
...
3. **이름**과 **설명**을 입력합니다.
4. **플랫폼**에서 **Windows 10 이상**을 선택합니다.
5. **Microsoft Defender ATP** 설정에서 **디바이스가 머신 위험 점수나 그 아래에 있어야 함**을 기본 설정 수준으로 설정합니다. 위협 수준 분류는 [Microsoft Defender ATP에 의해 결정](https://docs.microsoft.com/windows/security/threat-protection/microsoft-defender-atp/alerts-queue)됩니다.
...
1. [Intune](https://go.microsoft.com/fwlink/?linkid=2090973)에 로그인합니다.
2. **디바이스 준수** > **정책**을 선택한 후 Microsoft Defender ATP 준수 정책을 선택합니다.
3. **할당**을 선택합니다.
