...
|-2016345598|0x87D10202|Syncml(514): SyncML 명령이 처리되기 전에 작업이 이미 취소되었으므로, 이 명령은 성공적으로 완료되지 않았습니다.|
|-2016345599|0x87D10201|Syncml(513): 받는 사람이 요청 SyncML 메시지에 사용된 SyncML 동기화 프로토콜의 지정된 버전을 지원하지 않거나 지원하기를 거부합니다.|
|-2016345600|0x87D10200|Syncml(512): 동기화 세션 동안 응용 프로그램 오류가 발생했습니다.|
|-2016345601|0x87D101FF|Syncml(511): 요청을 처리하는 동안 서버에서 심각한 오류가 발생했습니다.|
|-2016345602|0x87D101FE|Syncml(510): 메시지를 처리하는 동안 오류가 발생했습니다. 이 오류는 수신자 데이터 저장소의 실패와 관련이 있습니다.|
|-2016345603|0x87D101FD|Syncml(509): 향후 사용을 위해 예약되어 있습니다.|
|-2016345604|0x87D101FC|Syncml(508): 서버와 클라이언트의 현재 동기화 상태를 새로 고쳐야 하는 오류가 발생 했습니다.|
|-2016345605|0x87D101FB|Syncml(507): 오류 때문에 Atomic 요소 유형 내의 모든 SyncML 명령이 실패합니다.|
|-2016345606|0x87D101FA|Syncml(506): 메시지를 처리하는 동안 응용 프로그램 오류가 발생했습니다.|
...
|-2016345609|0x87D101F7|Syncml(503): 받는 사람이 임시 과부하나 받는 사람 유지 관리로 인해 현재 요청을 처리할 수 없습니다.|
...
|-2016345690|0x87D101A6|Syncml(422): LocURI의 CGI 스크립트 형식이 잘못되었으므로 요청한 명령이 서버에서 실패합니다.|
|-2016345691|0x87D101A5|Syncml(421): 지정된 검색 문법을 알려지지 않았기 때문에 요청한 명령이 서버에서 실패합니다.|
|-2016345692|0x87D101A4|Syncml(420): 받는 사람에게 동기화 데이터를 위한 저장 공간이 더 이상 남아 있지 않습니다.|
...
|-2016345697|0x87D1019F|Syncml(415): 지원 되지 않는 미디어 유형 또는 형식입니다.|
...
|-2016345703|0x87D10199|Syncml(409): 요청한 명령이 데이터의 클라이언트 버전과 서버 버전 사이의 업데이트 충돌 때문에 실패했습니다.|
|-2016345704|0x87D10198|Syncml(408): 예상한 메시지가 필요한 기간 내에 수신되지 않았습니다.|
