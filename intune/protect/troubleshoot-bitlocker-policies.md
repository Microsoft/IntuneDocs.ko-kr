...
- **장치 구성 정책** -endpoint protection을 관리 하는 장치 구성 프로필을 만들 때 Intune에서 특정 기본 제공 정책 옵션을 사용할 수 있습니다. 이러한 옵션을 찾으려면 [endpoint protection에 대 한 장치 프로필을 만들고](endpoint-protection-configure.md#create-a-device-profile-containing-endpoint-protection-settings) *플랫폼*에 대해 **Windows 10 이상을** 선택한 다음 *설정*에 대 한 **windows 암호화** 범주를 선택 합니다. 

   사용할 수 있는 옵션 및 기능에 대 한 자세한 내용은 [Windows 암호화](https://docs.microsoft.com/intune/endpoint-protection-windows-10#windows-encryption)를 참조 하세요.
...
BitLocker에 대해 자세히 알아보려면 다음 리소스를 참조 하세요.
...
장치 구성 정책을 사용 하 여 BitLocker를 구성 하는 경우 Intune 포털에서 정책의 상태를 확인할 수 있습니다.

1. [Microsoft Endpoint Manager 관리 센터](https://go.microsoft.com/fwlink/?linkid=2109431)에 로그인합니다.

2. **장치** > **구성 프로필** 을 선택 하 고 BitLocker 설정을 포함 하는 프로필을 선택 합니다.

3. 보려는 프로필을 선택한 후 **장치 상태**를 선택 합니다. 프로필에 할당 된 장치가 나열 되 고 *장치 상태* 열에 장치가 프로필을 성공적으로 배포 했음을 나타냅니다.
...
status 명령의 결과를 ![](./media/troubleshooting-bitlocker-policies/command.png)
...
BitLocker 정책이 장치에 성공적으로 배포 되 면 장치에서 BitLocker 설정: *HKEY_LOCAL_MACHINE \software\microsoft\policymanager\current\device\bitlocker*의 구성을 검토할 수 있는 다음 레지스트리 키를 확인 합니다. 아래 예를 살펴보세요.
...
- **디바이스가 Microsoft Intune에 제대로 등록되어 있습니까?** 그렇지 않은 경우 정책과 관련 된 모든 문제를 해결 하기 전에 해결 해야 합니다. Windows 등록 문제 해결에 대 한 도움말은 [여기](../enrollment/troubleshoot-windows-enrollment-errors.md)를 참조 하세요.

- **디바이스에 활성 네트워크 연결이 있습니까?** 장치가 비행기 모드에 있거나 꺼져 있거나, 사용자가 서비스가 없는 위치에 있는 경우에는 네트워크 연결이 복원 될 때까지 정책이 배달 되거나 적용 되지 않습니다.
...
- **전체 정책의 배포가 실패 하거나 적용 되지 않는 특정 설정에만 적용 되나요?** 일부 정책 설정만 적용 되지 않는 시나리오를 발견 한 경우 다음 사항을 확인 하십시오.
  ...
  2. **BitLocker는 모든 하드웨어에서 지원 되지 않습니다**.
     올바른 버전의 Windows를 사용 하는 경우에도 기본 장치 하드웨어가 BitLocker 암호화에 대 한 요구 사항을 충족 하지 못할 수 있습니다. Windows 설명서에서 [BitLocker에 대 한 시스템 요구 사항을](https://docs.microsoft.com/windows/security/information-protection/bitlocker/bitlocker-overview#system-requirements) 확인할 수 있지만 확인 해야 할 주요 사항은 장치에 호환 되는 TPM 칩 (1.2 이상)과 TCG(신뢰할 수 있는 컴퓨팅 그룹) (TCG) 규격 BIOS 또는 UEFI 펌웨어가 있는지 확인 하는 것입니다.

**조사 예제**

- Windows 10 장치에 BitLocker 정책을 배포 하면 **장치 암호화** 설정이 포털에 **오류** 상태를 표시 합니다.
