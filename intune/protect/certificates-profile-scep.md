...
> [!NOTE]
> iOS/iPadOS 디바이스에서 SCEP 인증서 프로필이 Wi-Fi 또는 VPN 프로필과 같은 추가 프로필과 연결되면, 디바이스는 각각의 추가 프로필에 대한 인증서를 받습니다. 이로 인해 iOS/iPadOS 디바이스는 SCEP 인증서 요청으로 전달된 여러 인증서를 갖게 됩니다.  단일 인증서가 필요한 경우 SCEP 인증서 대신 PKCS 인증서를 사용해야 합니다.  이는 SCEP 및 PKCS 인증서를 디바이스에 전달하는 방법이 다르기 때문입니다.
