...
- **디바이스의 데이터 스토리지 암호화**(Android 4.0 이상 또는 KNOX 4.0 이상): **필요**를 선택하면 디바이스의 데이터 스토리지가 암호화됩니다. **모바일 디바이스의 잠금을 해제하는 데 암호 필요** 설정을 선택하면 디바이스가 암호화됩니다. **구성되지 않음**(기본값)을 선택하면 이 설정은 규정 준수 또는 비준수에 대해 평가되지 않습니다.
