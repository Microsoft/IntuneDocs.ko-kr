---
title: 하이브리드 Active Directory 조인 디바이스 등록 - Windows Autopilot
titleSuffix: ''
description: Windows Autopilot를 사용하여 하이브리드 Active Directory 조인 디바이스를 Microsoft Intune에 등록합니다.
keywords: ''
author: ErikjeMS
ms.author: erikje
manager: dougeby
...
- [하이브리드 Azure Active Directory 조인 디바이스](https://docs.microsoft.com/azure/active-directory/devices/hybrid-azuread-join-plan)를 성공적으로 구성합니다.
...
일부 도메인에서는 컴퓨터를 만들 수 있는 권한이 컴퓨터에 부여되지 않습니다. 또한 도메인에서는 컴퓨터 개체를 만들 수 있는 권한이 위임되지 않은 모든 사용자 및 컴퓨터에 기본 제공 제한(기본값 10)이 적용됩니다. 따라서 하이브리드 Azure AD 조인 디바이스가 만들어진 조직 단위의 Intune 커넥터를 호스트하는 컴퓨터에 권한이 위임되어야 합니다.
...
8. **그룹 선택** 블레이드에서 **그룹 선택**을 선택하고, 디바이스 그룹 &gt; **선택**을 차례로 선택합니다.
