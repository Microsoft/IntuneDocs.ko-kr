...
1. [Intune](https://go.microsoft.com/fwlink/?linkid=2090973)에서 **디바이스 구성** > **관리** > **프로필** > **프로필 만들기**를 선택합니다.
2. 다음 속성을 입력합니다.

    - **이름**: 프로필에 대한 설명이 포함된 이름을 입력합니다. 나중에 쉽게 식별할 수 있도록 프로필 이름을 지정합니다. 예를 들어 **Android 엔터프라이즈 디바이스 소유자 디바이스에 대한 신뢰할 수 있는 인증서 프로필** 또는 **iOS 디바이스에 대한 신뢰할 수 있는 인증서 프로필**은 좋은 프로필 이름입니다.
...
3. [2단계: 신뢰할 수 있는 루트 CA 인증서 내보내기](#step-2-export-your-trusted-root-ca-certificate)에 저장한 인증서를 찾은 다음, **확인**을 선택합니다.
4. Windows 8.1 및 Windows 10 디바이스에 한해 신뢰할 수 있는 인증서의 **대상 저장소**를 선택합니다.

    - **컴퓨터 인증서 저장소 - 루트**(SCEP)
    - **컴퓨터 인증서 저장소 - 중간**(SCEP)
    - **사용자 인증서 저장소 - 중간**(PKCS,SCEP)

5. 작업이 완료되면 **확인**을 선택하고 **프로필 만들기** 창으로 돌아와서 **만들기**를 선택합니다.
