...
Microsoft Intune에는 조직의 디바이스에 배포할 수 있는 여러 이메일 설정이 포함됩니다. IT 관리자는 Office 365 및 Gmail 등 메일 서버에 연결하는 데 사용할 특정 설정으로 이메일 프로필을 만들 수 있습니다. 그러면 사용자가 모바일 디바이스에서 조직의 이메일 계정에 연결하고, 인증하고, 동기화할 수 있습니다. 이메일 프로필을 만들고 배포하면 여러 디바이스 간에 표준 설정이 적용됨을 확인할 수 있습니다. 그리고 올바른 이메일 설정을 모르는 최종 사용자의 지원 요청을 줄이는 데 도움이 됩니다.
...
- **인증서**: 메일 프로필을 만들 때 이전에 Intune에서 만든 인증서 프로필을 선택합니다. 이 인증서를 ID 인증서라고 합니다. 이는 사용자 디바이스의 연결이 허용되었음을 확인하기 위해 신뢰할 수 있는 인증서 프로필 또는 루트 인증서에 대해 인증하는 데 사용됩니다. 신뢰할 수 있는 인증서는 메일 연결을 인증하는 컴퓨터에 할당됩니다. 일반적으로 이 컴퓨터는 네이티브 메일 서버입니다.
