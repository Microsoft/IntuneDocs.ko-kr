...
Intune에는 앱 관리 및 디바이스 설정 구성 등 다양한 기능이 포함되어 있습니다. 이러한 기본 제공 기능 및 설정은 "Zebra 디바이스"라고도 알려진 Zebra Technologies에서 제조한 Android 디바이스를 관리하는 데 사용됩니다.
...
디바이스에서 Google Play 스토어로 이동하고 Microsoft에서 Intune 회사 포털 앱을 다운로드하여 설치합니다. Google Play에서 설치하는 경우 회사 포털 앱은 자동으로 업데이트를 가져오고 수정합니다.
...
[Android 디바이스를 등록](android-enroll.md)할 단계를 나열합니다. 많은 Zebra 디바이스가 있는 경우 [DEM(디바이스 등록 관리자) 계정](device-enrollment-manager-enroll.md)을 사용할 수 있습니다. 또한 DEM 계정을 사용하면 회사 포털 앱에서 등록 취소 옵션이 제거됨으로 사용자가 쉽게 등록 취소할 수 없습니다.
...
StageNow에서 프로필을 만들 때 마지막 단계에서 **MDM으로 내보내기**를 선택합니다. 이는 XML 파일을 생성합니다. 이 파일을 저장합니다. 이후 단계에서 필요합니다.
...
> - [추가](apps-add.md), [배포](apps-deploy.md), 업데이트 및 [모니터](apps-monitor.md) 앱에 대한 앱 관리 기능.
...
> [!NOTE]
> 각 디바이스에 하나의 프로필을 배포합니다. 디바이스에 배포할 여러 StageNow 프로필이 있는 경우 StageNow 프로필을 내보낸 다음, Intune에 추가하기 전에 설정을 단일 XML 파일로 결합합니다. 
> 
> 동일한 XML 파일에서 동일한 속성을 구성하는 두 가지 설정을 원하지 않습니다. 목표는 디바이스의 설정 간에 충돌을 방지하는 것입니다.
...
다음에 디바이스가 구성 업데이트를 확인할 때 MX 프로필이 디바이스에 배포됩니다. 디바이스가 등록될 때 Intune과 디바이스를 동기화한 다음, 약 8시간마다 동기화합니다. [Intune에서 동기화를 강제로 수행](device-sync.md)할 수도 있습니다. 또는 디바이스에서 **회사 포털 앱** > **설정** > **동기화**를 엽니다. 

> [!TIP]
> - 보안상의 이유로 저장한 후에는 프로필 XML 텍스트를 볼 수 없습니다. 텍스트가 암호화되어 별표(`****`)만 볼 수 있습니다. 참고로 Intune에 추가하기 전에 MX 프로필 복사본을 저장하는 것이 좋습니다.
> 
> - Zebra 디바이스에 할당된 후 프로필을 업데이트하려면 업데이트된 StageNow XML 파일을 만들고, 기존 Intune 프로필을 편집하고, 새 StageNow XML 파일을 추가합니다. 이 새 파일은 프로필의 이전 StageNow 정책을 덮어씁니다.
