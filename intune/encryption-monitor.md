...
ms.locfileid: "68375156"
---
# <a name="monitor-device-encryption"></a>모니터 디바이스 암호화  

Microsoft Intune 암호화 보고서는 관리형 디바이스의 암호화 상태를 자세히 확인할 수 있는 중앙 위치입니다. 디바이스의 암호화 상태에 관한 세부 정보를 확인하고 디바이스 복구 키를 관리하는 옵션을 찾을 수 있습니다. 사용할 수 있는 복구 키 옵션은 보고 있는 디바이스 유형에 따라 다릅니다.  
...
- **암호화 준비** – BitLocker 또는 FileVault 암호화와 같은 해당 암호화 기술을 지원하기 위한 디바이스의 준비 상태를 평가합니다. 디바이스는 다음과 같이 식별됩니다.  
  ...
  - **준비되지 않음**: 디바이스가 완전한 암호화 기능을 갖추고 있진 않지만 여전히 암호화를 지원합니다. 예를 들어 사용자가 수동으로 또는 TMP 없이 암호화를 허용하도록 설정할 수 있는 그룹 정책을 통해 Windows 디바이스를 암호화할 수 있습니다.
...
- **암호화 준비** - MDM 정책을 통해 암호화를 지원하기 위한 디바이스 준비 상태를 평가합니다.  
  ...
  예를 들면 다음과 같습니다. Windows 10 디바이스의 준비 상태가 ‘준비 안 됨’인 경우에도 암호화를 지원할 수 있습니다.  ‘준비’로 지정되려면 Windows 10 디바이스에 TPM 칩이 있어야 합니다.  TPM 칩이 없어도 암호화는 지원할 수 있습니다. (자세한 내용은 이전 섹션의 암호화 준비를 참조하세요.)  

- **암호화 상태** – OS 드라이브의 암호화 여부를 나타냅니다. Intune에서 디바이스 암호화 상태나 해당 상태로의 변경을 보고하는 데 최대 24시간 걸릴 수 있습니다.  
...
- **상태 세부 정보** – 디바이스의 암호화 상태에 대한 고급 정보입니다.  
  > [!NOTE]
  > 7월 릴리스 출시가 며칠 내에 완료될 때까지 FileVault의 지원이 제한됩니다. 롤아웃이 완료될 때까지 macOS의 디바이스 상태 정보와 디바이스 암호화 세부 정보가 암호화 보고서에 정확하게 표시되지 않을 수 있습니다.
  ...
  **macOS**:
  - 필수 구성 요소를 기다리는 중이므로 지금은 프로필을 설치할 수 없습니다.  
 
    *‘고려할 사항은 다음과 같습니다. 이 결과는 반드시 오류 상태를 나타내지는 않지만 디바이스에서의 타이밍으로 인한 임시 상태, 즉 복구 키의 에스크로를 설정해야만 디바이스에 암호화 요청을 보낼 수 있는 상태를 나타냅니다. 최근에 디바이스가 Intune에서 잠긴 상태로 유지되거나 체크 인되지 않았음을 나타낼 수도 있습니다. 마지막으로 디바이스가 연결(충전)될 때까지 FileVault 암호화가 시작되지 않으므로 사용자가 아직 암호화되지 않은 디바이스의 복구 키를 받을 수 있습니다’.*  

  - FileVault 프로필이 설치되었지만 디바이스에서 FileVault를 사용하도록 설정하지 않았습니다.  
  ...
  - FileVault를 사용자가 이미 사용하도록 설정하여 Intune에서 복구를 관리할 수 없습니다.  
  ...
  - MacOS Catalina 이상에서는 FileVault를 사용하려면 사용자가 해당 관리 프로필을 승인해야 합니다.  
    ...
    *‘고려할 사항은 다음과 같습니다. MacOS 버전 10.15(Catalina)부터 사용자 승인됨 등록 설정으로 인해 사용자가 FileVault 암호화를 수동으로 승인해야 합니다. 자세한 내용은 Intune 설명서에서 [사용자 승인됨 등록](macos-enroll.md)을 참조하세요’.   

  - iOS 디바이스가 NotNow(잠김)를 반환했습니다.  

    *‘고려할 사항은 다음과 같습니다. 디바이스가 현재 잠겨 있고 Intune에서 에스크로 또는 암호화 프로세스를 시작할 수 없습니다. 디바이스를 잠금 해제한 후 계속 진행할 수 있습니다’  .  
...
Intune은 개인 복구 키를 회전 및 복구하는 여러 옵션을 지원합니다. 키를 회전시키는 이유는 현재 개인 키가 분실되는 경우 위험해질 수 있기 때문입니다.  
