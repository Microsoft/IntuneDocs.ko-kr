...
- 저장 된 로그는 로그에 포함 된 개인 식별이 가능한 정보를 보호 하기 위해 암호화 됩니다.
...
이 섹션에서는 장치 관리자 (DA) 및 Samsung Knox 등록에 대해 설명 합니다. 자세한 내용은 [android 장치 관리자 등록](../enrollment/android-enroll-device-administrator.md) 을 참조 하 고, [Samsung Knox 모바일 등록을 사용 하 여 Android 장치를 자동으로 등록](../enrollment/android-samsung-knox-mobile-enroll.md)을 참조 하세요. 
...
| 최종 사용자가 앱 설치를 취소했습니다. (0xC7D14FB1) | 사용자가 명시적으로 앱을 제거했습니다. 사용자가 Android OS 설치 작업을 취소한 경우 이 오류가 반환됩니다. 사용자가 OS 설치 프롬프트가 표시된 경우 취소 단추를 눌렀거나 프롬프트와 별개로 클릭했습니다. 이 오류는 DA 시나리오에 대해서만 반환됩니다. KNOX 시나리오의 경우 사용자에게 설치를 묻지 않습니다. 자동으로 수행될 수 있습니다. Intune은 사용자가 클릭하여 다시 시도할 수 있는 알림을 제공합니다. 앱이 사용 가능한 앱인 경우 알림을 해제할 수 있습니다. 그러나 앱이 필요한 경우 해제할 수 없습니다. 사용자에 게 설치를 취소 하지 않도록 요청 합니다. |
| 파일 다운로드 프로세스가 예기치 않게 중지되었습니다. (0xC7D15015) | 다운로드 프로세스가 완료되기 전에 OS에서 중지했습니다. 디바이스에 배터리가 부족하거나 다운로드가 너무 오래 걸리는 경우 이 오류가 발생할 수 있습니다. 이 오류는 DA 시나리오에 대해서만 반환됩니다. KNOX 시나리오의 경우 사용자에게 설치를 묻지 않습니다. 자동으로 수행될 수 있습니다. Intune은 사용자가 클릭하여 다시 시도할 수 있는 알림을 제공합니다. 앱이 사용 가능한 앱인 경우 알림을 해제할 수 있습니다. 그러나 앱이 필요한 경우 해제할 수 없습니다. 장치에 안정적인 네트워크 연결이 있는지 확인 합니다.  |
| 파일 다운로드 서비스가 예기치 않게 중지되었습니다. 다음에 디바이스가 동기화될 때 정책이 다시 시도됩니다. (0xC7D1507C) | 다운로드 프로세스가 완료되기 전에 OS에서 중지했습니다. 디바이스에 배터리가 부족하거나 다운로드가 너무 오래 걸리는 경우 이 오류가 발생할 수 있습니다. 이 오류는 DA 시나리오에 대해서만 반환됩니다. KNOX 시나리오의 경우 사용자에게 설치를 묻지 않습니다. 자동으로 수행될 수 있습니다. 수동으로 장치를 동기화 하거나 24 시간 동안 기다린 후 상태를 확인 합니다. |
| 앱을 제거 하지 못했습니다. (0xc7d14fb8) | 이 오류는 일반적인 제거 오류입니다. OS에서 앱을 제거 하지 못한 이유를 지정 하지 않았습니다. 일부 관리 앱은 단순히 제거할 수 없습니다. 제거에 실패 하는 경우 수동으로 앱을 제거 하 고 회사 포털 로그를 수집할 수 있는지 확인 합니다. |
| 업그레이드에 사용 되는 앱 설치 APK 파일이 장치에 있는 현재 앱의 서명과 일치 하지 않습니다. (0xc7d14fb7) | Android OS는 기존 버전에 서명 하는 데 사용 되는 인증서와 동일 하 게 업그레이드 버전의 서명 인증서를 요구 하는 제한 사항이 있습니다. 개발자가 동일한 인증서를 사용 하 여 새 버전에 서명할 수 없는 경우 기존 앱을 제거 하 고 기존 앱을 업그레이드 하는 대신 새 앱을 다시 배포 해야 합니다. |
| 최종 사용자가 앱 설치를 취소했습니다. (0xc7d14fb9) | 메시지가 표시 되 면 Intune 배포 앱을 수락 하 고 앱을 설치 하도록 사용자에 게 교육 합니다. |
| 설치 중 프로세스가 다시 시작되어 앱 제거가 취소되었습니다. (0xc7d14fbc) | OS가 앱 설치 프로세스를 종료 했거나 장치가 다시 시작 되었습니다. 이 오류가 다시 발생 하는 경우 설치 및 회사 포털 로그 수집을 다시 시도 합니다. |
| 앱 설치 APK 파일이 서명 되지 않았기 때문에 설치할 수 없습니다. (0xc7d14fb6) | 기본적으로 Android OS는 앱에 서명 해야 합니다. 배포 전에 앱이 서명 되었는지 확인 합니다. |
...
| iOS 디바이스가 현재 사용 중입니다. (0x87D11388) | iOS 디바이스가 사용 중이므로 오류가 발생했습니다. 장치가 잠겼습니다. 사용자는 앱을 설치 하기 위해 장치의 잠금을 해제 해야 합니다. |
| 앱을 설치하지 못했습니다. (0x87D13B64) | 앱 설치 오류가 발생했습니다. 이 오류를 해결하려면 iOS 콘솔 로그가 필요합니다. |
| 앱이 관리되지만 만료되었거나 사용자가 제거했습니다. (0x87D13B66) | 사용자가 앱을 명시적으로 제거 했거나 앱이 만료 되었지만 다운로드 하지 못했거나 앱 검색은 장치의 응답과 일치 하지 않습니다. 또한 이 오류는 iOS 9.2.2 플랫폼 버그를 기반으로 발생할 수 있습니다. |
...
| 사용자가 앱 설치 제안을 거부했습니다. (0x87D13B62) | 초기 앱을 설치하는 동안 사용자가 취소를 클릭했습니다. 사용자에 게 다음 번에 설치 요청을 수락 하도록 요청 합니다. |
| 사용자가 앱 업데이트 제안을 거부했습니다. (0x87D13B63) | 최종 사용자가 업데이트 프로세스 중 취소를 클릭했습니다. 필요에 따라 배포 하거나 업그레이드 프롬프트에 동의 하도록 사용자에 게 교육 합니다. |
...
| Apple 오류로 인해 라이선스를 할당 하지 못했습니다. ' 남은 VPP 라이선스 없음 ' (0x87d13b7e) | 이 동작은 의도된 것입니다. 이 문제를 해결 하려면 추가 VPP 라이선스를 구매 하거나 더 이상 대상이 아닌 사용자의 라이선스를 회수 합니다. |
| 앱 설치 오류 12024: 알 수 없는 원인입니다. (0x87d13b6e) | Apple에는 설치가 실패 한 이유를 확인 하는 데 충분 한 정보가 제공 되지 않았습니다. 보고할 내용이 없습니다. |
| 필요한 앱 구성 정책이 없습니다. 정책이 동일한 그룹을 대상으로 하는지 확인 하세요. (0x87d13b7f) | 앱에 앱 구성이 필요 하지만 앱 구성이 대상으로 지정 되지 않았습니다. 관리자는 앱이 대상으로 하는 그룹에도 그룹을 대상으로 하는 필수 앱 구성이 있는지도 확인 해야 합니다. |
| 장치 VPP 라이선스는 iOS 9.0 이상 장치에만 적용 됩니다. (0x87d13b69) | 영향을 받는 iOS 장치를 iOS 9.0 이상으로 업그레이드 합니다. |
| 응용 프로그램이 장치에 설치 되어 있지만 관리 되지 않습니다. (0x87d13b8f) | 이 오류는 LOB 앱에만 발생 합니다. 앱이 Intune 외부에 설치 되었습니다. 이 오류를 해결 하려면 장치에서 앱을 제거 합니다. 다음 번에 장치 동기화가 수행 되 면 장치는 Intune에서 앱을 설치 해야 합니다. |
| 사용자가 앱 관리를 거부 했습니다 (0x87d13b68). | 앱 관리에 동의 하도록 사용자에 게 요청 합니다. |
| 알 수 없는 오류입니다. (0x87d1279d) | 이 오류는 iOS 스토어 앱에 발생 하지만 오류 시나리오를 알 수 없습니다. |
| 최신 버전의 앱을 이전 버전에서 업데이트 하지 못했습니다. (0x87D13B9D) | 이 오류 메시지는 앱이 설치 되 고 관리 되지만 장치에 잘못 된 버전이 있는 경우 표시 됩니다. 이러한 상황에는 장치에서 앱을 업데이트 하는 명령을 받았지만 새 버전이 아직 설치 및 보고 되지 않은 경우에 포함 됩니다. 이 오류는 업그레이드가 배포 된 후 장치에 대 한 첫 번째 체크 인에 대해 보고 되며, 장치가 새 버전을 설치 하거나 다른 오류로 인해 실패할 때까지 발생 합니다.  |
