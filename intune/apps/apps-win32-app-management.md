...
4. **앱 추가** 창에 제공된 드롭다운 목록에서 **Windows 앱(Win32)** 를 선택합니다.
    ...
    ![앱 추가 창 - 형식 추가 드롭다운 상자 스크린샷](./media/apps-win32-app-management/apps-win32-app-01.png)

### <a name="step-2-upload-the-app-package-file"></a>2단계: 앱 패키지 파일 업로드

1. **앱 추가** 창에서 **앱 패키지 파일**을 선택하여 파일을 선택합니다. 앱 패키지 파일 창이 표시됩니다.

    ![앱 패키지 파일 창 스크린샷](./media/apps-win32-app-management/apps-win32-app-02.png)
...
2. **Install 명령**을 구성하려면 전체 설치 명령줄을 추가하여 앱을 설치합니다. 
...
3. **Uninstall 명령**을 구성하려면 전체 제거 명령줄을 추가하여 앱의 GUID를 기반으로 앱을 제거합니다. 

    예:  `msiexec /x “{12345A67-89B0-1234-5678-000001000000}”`

4. **설치 동작**을 **시스템** 또는 **사용자**로 설정합니다.
...
5. **디바이스 다시 시작 동작**을 구성하려면 다음 옵션 중 하나를 선택합니다.
    - **반환 코드에 따라 동작 결정**: [반환 코드](~/apps/apps-win32-app-management.md#step-7-configure-app-return-codes) 구성 설정에 따라 디바이스를 다시 시작하려면 이 옵션을 선택합니다.
...
3. **추가**를 클릭하여 **요구 사항 규칙 추가** 창을 표시하고 추가 요구 사항 규칙을 구성합니다. **요구 사항 유형**을 선택하여 요구 사항에 대한 유효성을 검사하는 방법을 결정하는 데 사용할 규칙 유형을 선택합니다. 요구 사항 규칙은 파일 시스템 정보, 레지스트리 값 또는 PowerShell 스크립트를 기반으로 할 수 있습니다. 
...
5. **할당** 창에서 사용자 또는 디바이스에 따라 할당하도록 선택합니다. 할당을 선택하는 경우 **최종 사용자 환경**을 선택할 수도 있습니다. **최종 사용자 환경**을 선택하면 **최종 사용자 알림**, **다시 시작 유예 기간**, **가용성** 및 **설치 최종 기한**을 설정할 수 있습니다. 자세한 내용은 **Win32 앱 가용성 및 알림 설정**을 참조하세요.
6. 포함된 그룹 선택을 완료하려면 **확인**을 선택합니다.
7. 이 앱 할당에서 영향을 주지 않도록 사용자 그룹을 제외하려면 **그룹 제외**를 선택합니다.
8. **그룹 추가** 창에서 **확인**을 선택합니다.
9. 앱 **할당** 창에서 **저장**을 선택합니다.
...
1. Intune에서 **앱** > **모든 앱**을 차례로 선택하여 추가된 클라이언트 앱 목록을 확인합니다. 
...
4. 앱 창에서 **할당** > **그룹 추가**를 선택합니다. 
5. **할당 유형**을 **필수**로 설정합니다. 앱 가용성은 할당 유형에 따라 설정할 수 있습니다. **할당 유형**은 **필수**, **등록된 디바이스에 사용 가능** 또는 **제거**일 수 있습니다.
6. **포함된 그룹**을 선택하여 앱이 할당될 사용자 그룹을 결정합니다. **할당** 창이 표시됩니다.
7. **이 앱을 모든 사용자에게 필수로 설정**을 **예**로 설정합니다.
...
8. **최종 사용자 환경** 옵션을 수정하려면 **편집**을 선택합니다.
9. **편집 할당** 창에서 **최종 사용자 알림**을 **모든 알림 메시지 표시**로 설정합니다. **최종 사용자 알림**은 **모든 알림 메시지 표시**, **컴퓨터 다시 시작에 대한 알림 메시지 표시** 또는 **모든 알림 메시지 숨기기**로 설정할 수 있습니다.
10. **앱 가용성**을 **특정 날짜 및 시간**으로 설정하고 날짜 및 시간을 선택합니다. 이 날짜 및 시간은 앱이 최종 사용자 디바이스에 다운로드되는 시기를 지정합니다. 
11. **앱 설치 최종 기한**을 **특정 날짜 및 시간**으로 설정하고 날짜 및 시간을 선택합니다. 이 날짜 및 시간은 앱이 최종 사용자 디바이스에 설치되는 시기를 지정합니다. 동일한 사용자 또는 디바이스에 대해 둘 이상의 할당을 수행하면 가능한 가장 빠른 시간을 기준으로 앱 설치 최종 기한 시간이 선택됩니다.
12. **다시 시작 유예 기간** 옆의 **사용하도록 설정**을 클릭합니다. 디바이스에서 앱 설치가 완료되는 즉시 다시 시작 유예 기간이 시작됩니다. 사용하지 않도록 설정하면 경고 메시지 없이 디바이스를 다시 시작할 수 있습니다. <br>다음 옵션을 사용자 지정할 수 있습니다.
...
13. **확인** > **확인** > **확인** > **저장**을 클릭하여 할당을 추가합니다.
