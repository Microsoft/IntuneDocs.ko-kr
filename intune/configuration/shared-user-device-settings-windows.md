...
이러한 설정은 [Sharedpc CSP](https://docs.microsoft.com/windows/client-management/mdm/sharedpc-csp)를 사용 합니다.
...
- **절전 모드 시간 제한(초)** : 디바이스가 절전 모드로 전환되기 전에 비활성 시간(초)(0-18000)의 수를 입력합니다. `0` 장치가 절전 모드로 전환 되지 않음을 의미 합니다. 시간을 설정하지 않으면 디바이스는 3600초(60분) 후 절전 모드로 전환합니다.
...
- **빠른 처음 로그인**: 사용자에 게 빠른 첫 로그인 환경을 제공 하도록 **사용** 을 선택 합니다. 이 **기능을 사용 하도록 설정**하면 장치는 새 비 관리자 Azure AD 계정을 미리 구성 된 후보 로컬 계정에 자동으로 연결 합니다. 빠른 첫 로그인 환경을 방지 하려면 **사용 안 함** 을 선택 합니다. **구성되지 않음**(기본값)은 이 설정을 Intune에서 관리되지 않는 상태로 유지하고, 디바이스에서 이 설정을 제어하는 모든 정책을 푸시하지 않습니다.
