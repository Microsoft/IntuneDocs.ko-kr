...
참고 데이터 로밍 (팁 또는 중요 한 참고 사항: 고객 혼동에 도움이 됨)이 필요 합니다 .이 설정은 대상 장치의 관리 프로필에 표시 되지 않습니다. 이 설정은 원격 장치 작업으로 처리 되기 때문에, 장치에서 데이터 로밍 상태가 변경 될 때마다 Intune 서비스에 의해 다시 차단 됩니다. 관리 프로필에는 표시 되지 않지만 관리 콘솔에서 보고의 성공으로 표시 되는 경우 작동 합니다. 
- **데이터 로밍**: **차단**을 선택하면 셀룰러 네트워크를 통한 데이터 로밍이 제한됩니다. **구성되지 않음**(기본값)을 통해 디바이스가 셀룰러 네트워크에 있을 때 데이터 로밍이 허용됩니다.

  > [!IMPORTANT]
  > 이 설정은 원격 장치 작업으로 처리 됩니다. 따라서이 설정은 장치의 관리 프로필에 표시 되지 않습니다. 장치에서 데이터 로밍 상태가 변경 될 때마다 Intune 서비스에서 **데이터 로밍을** 차단 합니다. Intune에서 보고 상태가 성공으로 표시 되는 경우 해당 설정이 장치의 관리 프로필에 표시 되지 않더라도 작동 한다는 것을 알 수 있습니다.
...
- **개인 핫스폿**: **블록**은 모든 디바이스 동기화마다 사용자 디바이스의 개인 핫스폿을 해제합니다. 이 설정은 일부 통신사와 호환되지 않을 수 있습니다. **구성되지 않음**(기본값)을 사용하면 사용자가 설정한 기본값으로 개인 핫스폿 구성을 유지합니다.
  ...
  > [!IMPORTANT]
  > 이 설정은 원격 장치 작업으로 처리 됩니다. 따라서이 설정은 장치의 관리 프로필에 표시 되지 않습니다. 장치에서 개인 핫스팟 상태가 변경 될 때마다 **개인 핫스팟** 은 Intune 서비스에 의해 차단 됩니다. Intune에서 보고 상태가 성공으로 표시 되는 경우 해당 설정이 장치의 관리 프로필에 표시 되지 않더라도 작동 한다는 것을 알 수 있습니다.
