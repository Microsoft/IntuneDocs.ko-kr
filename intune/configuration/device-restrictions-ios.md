...
> [!IMPORTANT]
> 사용자가 등록 한 장치에서 암호 설정을 구성 하면 **단순 암호** 설정이 자동으로 **차단**으로 설정 되 고 6 자리 PIN이 적용 됩니다.
>
> 예를 들어 **암호 만료** 설정을 구성 하 고 사용자가 등록 한 장치에이 정책을 푸시합니다. 장치에서 다음과 같은 상황이 발생 합니다.
>
> - **암호 만료** 설정은 무시 됩니다.
> - @No__t-0 또는 `1234`과 같은 단순한 암호는 허용 되지 않습니다.
> - 6 자리 pin이 적용 됩니다.
...
- **최소 암호 길이**: 사용자가 입력해야 하는 최소 길이를 입력합니다(4~14자 사이). 사용자 등록 장치에서 4 자에서 6 자 사이의 길이를 입력 합니다.
  ...
  > [!NOTE]
  > 사용자가 등록 한 장치의 경우 6 자리 보다 긴 PIN을 설정할 수 있습니다. 그러나 장치에는 6 자리 이하의 숫자만 적용 됩니다. 예를 들어 관리자는 최소 길이를 `8`으로 설정 합니다. 사용자가 등록 한 장치에서 6 자리 PIN을 설정 해야 합니다. Intune은 사용자가 등록 한 장치에서 6 자리를 초과 하는 PIN을 강제 적용 하지 않습니다.
...
- **앱 목록 유형**: 표시 하거나 숨길 앱 목록을 만듭니다. 옵션은 다음과 같습니다.
...
- **Wi-fi 상태 수정**: **구성 되지 않음** (기본값)을 사용 하면 사용자가 장치에서 wi-fi를 켜고 끌 수 있습니다. **Block** 은 wi-fi 켜기 또는 끄기를 방지 합니다.
