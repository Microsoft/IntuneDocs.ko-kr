...
이 도구를 사용하여 설정을 구성 프로필로 내보낼 수 있습니다. Intune에서 이 파일을 가져온 다음, macOS 사용자 및 디바이스에 프로필을 할당합니다. 일단 할당되면 설정이 배포됩니다. 또한 이 설정으로 조직의 macOS에 대한 기준 또는 표준도 만들어집니다.
...
- **구성 프로필 파일**: Apple Configurator 또는 Apple Profile Manager를 사용하여 만든 구성 프로필을 찾아봅니다. 가져온 파일은 **파일 내용** 영역에 표시됩니다.
  ...
  `.mobileconfig` 파일에 디바이스 토큰을 추가할 수도 있습니다. 디바이스 토큰은 디바이스별 정보를 추가하는 데 사용됩니다. 예를 들어 일련 번호를 표시하려면 `{{serialnumber}}`를 입력합니다. 디바이스에서 텍스트는 디바이스마다 고유한 `123456789ABC`와 유사하게 표시됩니다. 변수를 입력할 때 `{{ }}` 중괄호를 사용해야 합니다. [앱 구성 토큰](../apps/app-configuration-policies-use-ios.md#tokens-used-in-the-property-list)에는 사용할 수 있는 변수 목록이 포함됩니다. `deviceid` 또는 기타 디바이스 관련 값을 사용할 수도 있습니다.
...
[iOS/iPadOS 디바이스에서 프로필을 만드는](../custom-settings-ios.md) 방법을 참조하세요.
